        ...
        <table class="mainPage" id="blist">
        	...
        	<c:forEach items="${list }" var="at">
        		<c:if test="${count % 3 == 1 }">
					...
					<td>
						<a href="bdetail.do?b_no=${at.atch_parent_no }">
						<img src="resources/board_upfiles/${at.atch_file_name }" class="openBtn">
						</a>
					</td>
					<c:if test="${count > 1 and count % 3 == 0 }">
						</tr>
					</c:if>
					<c:set var="count" value="${count+ 1 }"/>
			</c:forEach>
			<c:if test="${count ne 4 }">
				<c:forEach var="n" begin="${ count }" step="1" end="3">
					<td>&nbsp;</td>
